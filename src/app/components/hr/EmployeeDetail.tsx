  ...
  if (!employee) {
    return (
      <div className="rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        <div className="text-lg font-semibold text-slate-900">Aucun employé sélectionné</div>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={onBack}
          className="flex items-center gap-3 text-sm font-medium text-slate-500 hover:text-slate-700 transition-colors"
        >
          <ChevronLeft size={15} />
          Annuaire
        </button>
              ...
              <div className="p-6">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {[
                      ...
                      <div key={item.label} className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                        <div className="flex items-center gap-3 text-slate-700 mb-2">{item.icon}<span className="text-sm font-semibold">{item.label}</span></div>
                ...
                <TabsContent value="documents">
                  ...
                  <div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-4">
                    <div className="text-sm font-semibold text-slate-900 mb-4">Documents récents</div>
                    <div className="space-y-3">
                      {docHistory.map((d, i) => (
                        <div key={i} className="flex items-center gap-3 rounded-2xl bg-white p-4">
                          ...
                          <span className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-700" style={{ color: "#1B3A6B" }}>{d.type}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={stat.label} className="rounded-3xl bg-slate-50 p-4 text-center">
