...
const radarConfig = { score: { label: "Score", color: "var(--chart-1)" } };
const barConfig = { tasks: { label: "Tâches", color: "var(--chart-2)" } };
      ...
      <Dialog open={!!selectedReq} onOpenChange={(open) => !open && setSelectedReq(null)}>
        <DialogContent className="sm:max-w-md bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl border-white/40 dark:border-slate-800/50">
          <DialogHeader>
            ...
            <DialogDescription>
              Vous êtes sur le point de <strong className={reviewAction === "approved" ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}>{reviewAction === "approved" ? "valider" : "refuser"}</strong> la demande de {reqObj?.name}.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
              <div className="text-sm font-medium text-slate-500 mb-1">Détails de la demande</div>
              <div className="text-sm font-bold text-slate-900 dark:text-white">{reqObj?.type}</div>
              <div className="text-xs text-slate-600 dark:text-slate-400 mt-1">{reqObj?.dates}</div>
            </div>
            <div>
